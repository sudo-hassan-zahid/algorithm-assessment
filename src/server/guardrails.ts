...
export async function inspectOwnedOrder(requestId: string, orderId: string) {
  const [order] = await db
    ...
    .innerJoin(
      supportRequests,
      eq(supportRequests.customerId, orders.customerId),
    )
...
export async function executeRefund(
  ...
  return db.transaction((tx) =>
    executeRefundInTransaction(tx, requestId, orderId, amount, actor),
  );
}
...
export async function executeRefundInTransaction(
  ...
  if (!order)
    throw new AppError(
      "ORDER_NOT_OWNED",
      "Order was not found for this customer",
      403,
    );
...
export async function executeCancellation(
  ...
  return db.transaction((tx) =>
    executeCancellationInTransaction(tx, requestId, orderId, actor),
  );
}
...
export async function executeCancellationInTransaction(
  ...
  if (!cancelled)
    throw new AppError(
      ...
      "Order changed while cancellation was being processed",
      409,
    );
