...
export async function createEscalation(input: EscalationInput) {
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.insert(actionEvents).values({
      ...
      actorType: "AGENT",
      eventType: "ESCALATED",
      details: { escalationId: escalation.id, action: input.action, reason: input.reason },
    });
    return escalation;
  });
}
...
export async function reviewEscalation(
  ...
  return db.transaction(async (tx) => {
    ...
    if (!escalation) throw new AppError("ESCALATION_NOT_FOUND", "Escalation not found", 404);
    if (escalation.status !== "PENDING") {
      throw new AppError("ALREADY_REVIEWED", `This escalation is already ${escalation.status.toLowerCase()}`, 409, {
    ...
    if (!escalation.order_id) {
      throw new AppError("ORDER_REQUIRED", "This action has no verified order and cannot be approved", 422);
    }
    if (escalation.action === "REFUND") {
      if (!escalation.proposed_amount) throw new AppError("AMOUNT_REQUIRED", "Refund amount is required", 422);
      ...
      await executeCancellationInTransaction(tx, escalation.request_id, escalation.order_id, {
        type: "REVIEWER",
        id: reviewer,
      });
    } else {
      throw new AppError("UNSUPPORTED_ACTION", "This action cannot be executed by the current system", 422);
    }

    const updated = await tx
      .update(escalations)
      .set({ status: "APPROVED", reviewedBy: reviewer, reviewedAt: new Date(), updatedAt: new Date() })
      .where(and(eq(escalations.id, escalationId), eq(escalations.status, "PENDING")))
      .returning();
    if (!updated.length) throw new AppError("ALREADY_REVIEWED", "Escalation was reviewed concurrently", 409);
    await tx
      .update(supportRequests)
