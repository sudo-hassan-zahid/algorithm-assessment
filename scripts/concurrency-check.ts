...
async function main() {
  ...
  try {
    ...
    await db.insert(supportRequests).values([
      ...refundRequestIds.map((id) => ({
        id,
        customerId,
        message: `Refund ${refundOrderId}`,
        status: "PROCESSING" as const,
      })),
      {
        id: approvalRequestId,
        customerId,
        message: `Cancel ${cancelOrderId}`,
    ...
    const refundAttempts = await Promise.allSettled(
      refundRequestIds.map((requestId) =>
        executeRefund(requestId, refundOrderId, "100.00", {
          type: "REVIEWER",
          id: "probe",
        }),
      ),
    ...
    const [refundTotal] = await db
      .select({ value: count() })
      .from(refunds)
      .where(eq(refunds.orderId, refundOrderId));
    if (
      refundAttempts.filter((result) => result.status === "fulfilled")
        .length !== 1 ||
      refundTotal.value !== 1
    ) {
    ...
    if (
      approvalAttempts.filter((result) => result.status === "fulfilled")
        .length !== 1 ||
      cancellationEvents.length !== 2
    ) {
    ...
    await db
      .delete(actionEvents)
      .where(inArray(actionEvents.requestId, requestIds));
    ...
    await db
      ...
      .where(inArray(supportRequests.id, requestIds));
    await db
      .delete(orders)
      .where(inArray(orders.id, [refundOrderId, cancelOrderId]));
